...
fisher_fdp = imputed_fdp = cauchy_fdp = IRT_fdp = trunct_fdp = hm_fdp = matrix(0,reps,length(sig))
fisher_etp = imputed_etp = cauchy_etp = IRT_etp = trunct_etp = hm_etp = matrix(0,reps,length(sig))
...
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','Fisher','Cauchy + Imputed','Cauchy'),each=length(sig)))
...
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','Fisher','Cauchy + Imputed','Cauchy'),each=length(sig)))
