...
d = 30
nmax = 20
rratio = c(0.05,0.1,0.2,0.3,0.5,0.6,0.7,0.8)#nmin/nmax
...
fedeval_mean_fdp = p2e_mean_fdp = p2e_ni_mean_fdp = fedeval_star_mean_fdp = fed_naive_fdp = pooled_fdp = matrix(0,reps,length(rratio))
fedeval_mean_etp = p2e_mean_etp = p2e_ni_mean_etp = fedeval_star_mean_etp = fed_naive_etp = pooled_etp = matrix(0,reps,length(rratio))

for(k in 1:length(rratio)){
  kk = nmin[k]
  set.seed(1)
  co = sample(kk:nmax,m,replace = TRUE)
  h_id = vector("list", length = d)
  for(i in 1:m){
    set.seed(i)
    h_id[[i]] = sample(1:d,co[i],replace = FALSE)
  }
  md_id = sapply(1:d,function(i) which(sapply(h_id, function(x) i %in% x)))
  ...
  for(r in 1:reps){
    ...
    for(ii in 1:m){
      ...
      estat_n[ii]=sum(unlist(sapply(1:d,
                                    function(i) (md[i]/nmax)*bh_decisions[[i]][which(md_id[[i]]==ii)]/(alphd[i]*max(sum(bh_decisions[[i]],1))))))
    ...
    fed_ebh_star = ebh.func(estat_ni,alph)$de
    fedeval_star_mean_fdp[r,k] = sum((1-theta)*fed_ebh_star)/max(sum(fed_ebh_star),1)
    fedeval_star_mean_etp[r,k] = sum(theta*fed_ebh_star)/max(sum(theta),1)
...
plotdata1<- as.data.frame(c(colmeans(fedeval_mean_fdp),colmeans(fedeval_star_mean_fdp),
                            colmeans(p2e_mean_fdp),colmeans(p2e_ni_mean_fdp),colmeans(pooled_fdp),
                            colmeans(fed_naive_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','IRT*','P2E','P2E*','Fisher','Naive'),
                              each=length(rratio)))
