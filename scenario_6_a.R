...
rho1 = 0
rho2 = 0
sig = c(1.1,1.2,1.3,1.4,1.5)

fisher_fdp = imputed_fdp = cauchy_fdp = IRT_fdp = eprod_fdp = trunct_fdp = hm_fdp = matrix(0,reps,length(sig))
fisher_etp = imputed_etp = cauchy_etp = IRT_etp = eprod_etp = trunct_etp = hm_etp = matrix(0,reps,length(sig))

alphd = rep(0.01,nd)
U = chol(rho1*matrix(1,m,m)+(1-rho1)*diag(m))
V = chol(rho2*matrix(1,nd,nd)+(1-rho2)*diag(nd))

for(rr in 1:length(sig)){
  ...
  for(r in 1:reps){
    ...
    theta = 1*(p>null_prop)
    x = MU+t(U)%*%y%*%V
    pvals = sapply(1:nd,function(i) pnorm(x[,i],0,sig[rr],lower.tail = FALSE))
...
plotdata1<- as.data.frame(c(colmeans(eprod_fdp),colmeans(fisher_fdp),
                            colmeans(imputed_fdp),colmeans(cauchy_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT*','Fisher','Fisher + Imputed','Cauchy'),each=length(sig)))
plotdata1$d<- rep(sig,4)
g1<-ggplot()+geom_line(data=plotdata1,aes(x=d,y=FDR,color=type),size=1)+
  ...
  ylab('FDP')+theme_bw()+xlab(TeX('$\\sigma$'))+
...
plotdata2<- as.data.frame(c(colmeans(eprod_etp),colmeans(fisher_etp),
                            colmeans(imputed_etp),colmeans(cauchy_etp)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT*','Fisher','Fisher + Imputed','Cauchy'),each=length(sig)))
plotdata2$d<- rep(sig,4)
g2<-ggplot()+geom_line(data=plotdata2,aes(x=d,y=etp,color=type),size=1)+
  ...
  ylab('ETP')+theme_bw()+xlab(TeX('$\\sigma$'))+
  theme(legend.position='top',legend.title=element_blank(),
        legend.background = element_rect(fill="white",
                                         size=1, linetype="solid", colour ="black"),
        legend.text=element_text(size=12),
