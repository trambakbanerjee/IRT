...
ehybrid_fdp = e_fdp = eprod_fdp = p2e_fdp = p2eprod_fdp = pooled_fdp = matrix(0,reps,length(df))
ehybrid_etp = e_etp = eprod_etp = p2e_etp = p2eprod_etp = pooled_etp = matrix(0,reps,length(df))
...
for(dd in 1:length(df)){
  for(r in 1:reps){
    ...
    mu = sapply(1:m,function(i) 0*(p[i]<=null_prop)+rnorm(1,-3,1)*(p[i]>null_prop & p[i]<=0.9)+
                  rnorm(1,3,1)*(p[i]>0.9))
    y = matrix(rt(m*nd,df[dd]),nd,m)
    theta = 1*(p>null_prop)
    x = sapply(1:nd,function(i) y[i,]+mu)
...
plotdata1<- as.data.frame(c(colmeans(e_fdp),colmeans(eprod_fdp),
                            ...
                            colmeans(pooled_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','IRT*','IRT H','P2E','P2E*','Fisher'),each=length(df)))
plotdata1$d<- rep(df,6)
...
plotdata2<- as.data.frame(c(colmeans(e_etp),colmeans(eprod_etp),
                            ...
                            colmeans(pooled_etp)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','IRT*','IRT H','P2E','P2E*','Fisher'),each=length(df)))
plotdata2$d<- rep(df,6)
