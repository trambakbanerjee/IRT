...
ehybrid_fdp = e_fdp = eprod_fdp = imputed_fdp = cauchy_fdp = pooled_fdp = matrix(0,reps,length(rho2_vec))
ehybrid_etp = e_etp = eprod_etp = imputed_etp = cauchy_etp = pooled_etp = matrix(0,reps,length(rho2_vec))
...
plotdata1<- as.data.frame(c(colmeans(e_fdp),colmeans(eprod_fdp),
                            ...
                            colmeans(cauchy_fdp),colmeans(imputed_fdp),
                            colmeans(pooled_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','IRT*','IRT H','Cauchy','Cauchy + Imputed',
                                'Fisher'),each=length(rho2_vec)))
...
plotdata2<- as.data.frame(c(colmeans(e_etp),colmeans(eprod_etp),
                            ...
                            colmeans(cauchy_etp),colmeans(imputed_etp),
                            colmeans(pooled_etp)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','IRT*','IRT H','Cauchy','Cauchy + Imputed',
                                'Fisher'),each=length(rho2_vec)))
