...
# d2 studies are also equi-correlated but only give 0/1 decisions
...
g2<-ggplot()+geom_line(data=plotdata2,aes(x=d,y=etp,color=type),size=1)+
  ...
  ylab('ETP')+theme_bw()+xlab(TeX('$\\rho$'))+scale_x_continuous(limits = c(0, 0.9),
                                                                 breaks = rho2_vec) +
  theme(legend.position='top',legend.title=element_blank(),
        legend.background = element_rect(fill="white",
                                         size=1, linetype="solid", colour ="black"),
