...
fisher_fdp = imputed_fdp = cauchy_fdp = IRT_fdp = eprod_fdp = trunct_fdp = hm_fdp = matrix(0,reps,length(df))
fisher_etp = imputed_etp = cauchy_etp = IRT_etp = eprod_etp = trunct_etp = hm_etp = matrix(0,reps,length(df))
...
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT*','Fisher','Fisher + Imputed','Cauchy'),each=length(df)))
...
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT*','Fisher','Fisher + Imputed','Cauchy'),each=length(df)))
