...
imt_rejections =sapply(1:length(alph),function(i) sum(ebh[,i]))
sum(1*(n_rejections>0))
max(imt_rejections)

hyp_alpha0.1 = g[which(ebh[,10]==1)]
cc= sapply(1:8,function(i) which(hyp_alpha0.1%in%glist[[i]][bh_decisions[[i]]==1]))

dd1 = sapply(1:d,function(i) sum(bh_decisions[[i]]))
dd2 = sapply(1:d,function(i) length(cc[[i]]))
...
for(ii in 1:d){
  ...
  tt=sapply(1:length(glist[[ii]]),function(i) which(as.character(n$Var1)%in%glist[[ii]][i]))
  ee[ii,tt] = estat[[ii]]
