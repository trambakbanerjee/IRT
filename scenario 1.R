...
ehybrid_fdp = e_fdp = eprod_fdp = p2e_fdp = p2eprod_fdp = pooled_fdp = matrix(0,reps,length(d))
ehybrid_etp = e_etp = eprod_etp = p2e_etp = p2eprod_etp = pooled_etp = matrix(0,reps,length(d))
...
plotdata1<- as.data.frame(c(colmeans(e_fdp),colmeans(eprod_fdp),
                            ...
                            colmeans(pooled_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','IRT*','IRT H','P2E','P2E*','Fisher'),each=length(d)))
plotdata1$d<- rep(d,6)
...
plotdata2<- as.data.frame(c(colmeans(e_etp),colmeans(eprod_etp),
                            ...
                            colmeans(pooled_etp)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','IRT*','IRT H','P2E','P2E*','Fisher'),each=length(d)))
plotdata2$d<- rep(d,6)
