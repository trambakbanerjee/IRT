...
fedeval_mean_fdp = p2e_mean_fdp = fed_naive_fdp = pooled_fdp = matrix(0,reps,length(d))
fedeval_mean_etp = p2e_mean_etp = fed_naive_etp = pooled_etp = matrix(0,reps,length(d))
...
for(dd in 1:length(d)){
  ...
  for(r in 1:reps){
    set.seed(r)
    y = matrix(rnorm(m*d[10],0,1),d[10],m)
    sig = runif(d[10],0.75,2)
    p = runif(m)
    mu = sapply(1:m,function(i) 0*(p[i]<=null_prop)+rnorm(1,-3,1)*(p[i]>null_prop & p[i]<=0.9)+
                  rnorm(1,3,1)*(p[i]>0.9))
    theta = 1*(p>null_prop)
    x = sapply(1:nd,function(i) sig[i]*y[i,]+mu)
...
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','P2E','Fisher','Naive'),each=length(d)))
plotdata1$d<- rep(d,4)
...
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','P2E','Fisher','Naive'),each=length(d)))
plotdata2$d<- rep(d,4)
