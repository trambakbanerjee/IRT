...
e_fdp = p2e_fdp = matrix(0,reps,length(mixprop))
e_etp = p2e_etp = matrix(0,reps,length(mixprop))
...
plotdata1<- as.data.frame(c(colmeans(e_fdp),
                            colmeans(p2e_fdp)))
names(plotdata1)<-"FDR"
plotdata1$type<-as.factor(rep(c('IRT','P2E'),each=length(mixprop)))
plotdata1$d<- rep(mixprop,2)
...
plotdata2<- as.data.frame(c(colmeans(e_etp),
                            colmeans(p2e_etp)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('IRT','P2E'),each=length(mixprop)))
plotdata2$d<- rep(mixprop,2)
